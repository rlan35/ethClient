...
async function deploy() {
  ...
  headerHex = encodeBlock(headersData1[1])
  console.log(headerHex)
  response = await ethClient.methods.addBlockHeader(headerHex).send(options);
  ...
  let resp = await ethClient.methods.getTest().call(options);
...
async function deploy_contract(contractJson) {
  ...
  let contractOptions = { data: contractJson.bytecode };
